      ...
      parallel {
        ...
        stage('Quality Tests') {
          steps {
            ...
            sh 'docker tag nodeapp-dev:trunk praveendoodipala/nodeapp-dev:latest'
            sh 'docker push praveendoodipala/nodeapp-dev:latest'
          }
        }
      }
      post {
            ...
            steps {
                    retry(3) {
                        timeout(time:10, unit: 'MINUTES') {
                            sh 'docker tag nodeapp-dev:trunk praveendoodipala/nodeapp-prod:latest'
                            sh 'docker push praveendoodipala/nodeapp-prod:latest'
                            sh 'docker save praveendoodipala/nodeapp-prod:latest | gzip > nodeapp-prod-golden.tar.gz'
                        }
                    }
